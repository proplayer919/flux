...
class FluxDownloader:
    ...
    def __init__(self, config_dir: Optional[str] = None, images_dir: Optional[str] = None):
    ...
    def download_config(self, url: str, name: Optional[str] = None, force: bool = False) -> bool:
        """
        ...
        try:
            ...
            if config.name in existing_configs and not force:
                if not Confirm.ask(f"Configuration '{config.name}' already exists. Overwrite?"):
    ...
    def download_image(self, url: str, name: Optional[str] = None, force: bool = False) -> bool:
        """
        ...
        try:
            ...
            if not any(filename.endswith(ext) for ext in ['.tar.gz', '.tar.xz', '.tar']):
                filename += '.tar.gz'
            ...
            console.print(f"[dim]Size: {self._format_size(image_path.stat().st_size)}[/dim]")
    ...
    def _record_download(self, url: str, download_type: str, name: str, path: str) -> None:
