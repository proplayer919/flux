...
class ContainerConfig(BaseModel):
    ...
    custom_commands: List[str] = Field(
        default_factory=list, description="Custom commands to run"
    )
    ...
    allow_x11: bool = Field(default=False, description="Enable X11 forwarding")
    ...
    def validate_distribution(cls, v):
        ...
        if v.lower() not in supported_distros:
            raise ValueError(
                f"Unsupported distribution. Supported: {', '.join(supported_distros)}"
            )
        return v.lower()
    ...
    def validate_architecture(cls, v):
        ...
        if v not in supported_archs:
            raise ValueError(
                f"Unsupported architecture. Supported: {', '.join(supported_archs)}"
            )
        return v
...
class ConfigManager:
    ...
    def _prompt_distribution(self) -> str:
        ...
        while True:
            choice = Prompt.ask(
                "Select distribution",
                choices=[str(i) for i in range(1, len(distros) + 1)],
                default="1",
            )
    ...
    def _prompt_architecture(self) -> str:
        ...
        choice = Prompt.ask(
            "Select architecture",
            choices=[str(i) for i in range(1, len(archs) + 1)],
            default="1",
        )
    ...
    def _prompt_custom_commands(self) -> List[str]:
        ...
        console.print(
            "Enter custom commands to run during build (empty line to finish):"
        )
    ...
    def _prompt_environment_vars(self) -> Dict[str, str]:
        ...
        console.print(
            "Enter environment variables (format: KEY=VALUE, empty line to finish):"
        )
    ...
    def _prompt_volumes(self) -> List[str]:
        ...
        console.print(
            "Enter volume mounts (format: /host/path:/container/path, empty line to finish):"
        )
